...
function AddStudent(props) {
  ...
  const onSubmit = methods.handleSubmit((data) => {
    console.log(data);
    setloading(true);
    fetch("http://localhost:3000/api/clearance", {
      method: "POST",
      headers: {
      ...
      .then((data) => data.json())
      .then((final) => {
        if (final === "email does't exist in the database") {
          setError(final);
        } else {
          toast("You havxe succesfully add student to the system");
          // setError();
        }
        methods.reset();
        setloading(false);
    ...
    <>
      <FormProvider {...methods}>
        <form action="#" className="mt-8 grid grid-cols-6 gap-6 md:w-[500px]">
          ...
          <div className="col-span-5 ">
            <Input {...issue_validate} className={"text-[16px] py-3"} />
          </div>

          <div className="col-span-5 ">
            <Input {...issueDetail_validate} className={"text-[16px] py-3"} />
          </div>
